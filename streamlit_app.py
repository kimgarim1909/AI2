...
# Google Drive 파일 ID
file_id = '14M8fJQ2KHtfXDm-sDtuw5UaA9xiRbdMl'
...
def display_right_content(labels):
    ...
    for i, label in enumerate(labels[:3]):
        with cols[i]:
            st.image(f"https://i.ibb.co/4KTrK1W/2104345-526663-0148.jpg?text={label}", caption=f"이미지: {label}", use_column_width=True)

    # 2nd Row - YouTube Videos based on labels
    for i, label in enumerate(labels[:3]):
        with cols[i]:
            st.video("https://www.youtube.com/watch?v=NAPVkBAoQT8", start_time=0)
